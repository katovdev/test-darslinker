...
> = {
  ...
  thousand_students: (stats) => stats.totalStudents >= 1000,
  ten_k_revenue: (stats) => stats.totalRevenue >= 10000,
  high_rating: (stats) =>
    stats.averageRating >= 4.8 && stats.totalReviews >= 50,
  hundred_students: (stats) => stats.totalStudents >= 100,
  first_course: (stats) => stats.totalCourses >= 1,
  five_courses: (stats) => stats.totalCourses >= 5,
  ten_courses: (stats) => stats.totalCourses >= 10,
  early_adopter: (stats) => {
...
export const calculateAchievementProgress = (
  ...
  switch (type) {
    ...
    case "five_courses":
      return Math.min((stats.totalCourses / 5) * 100, 100);
    case "ten_courses":
...
export function getAllAchievementsWithProgress(
  ...
  return Object.entries(ACHIEVEMENT_DEFINITIONS).map(([type, definition]) => ({
    id: type,
    ...definition,
    unlockedAt: ACHIEVEMENT_CRITERIA[type](stats)
